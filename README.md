...
- 🔀 **CPA** — [CLIProxyAPI](https://github.com/router-for-me/CLIProxyAPI), one shared LLM proxy for all coding agents (pi, Claude, Codex), at `cpa.<DOMAIN>`. Its `/management.html` endpoint uses the [CPA Manager Plus](https://github.com/seakee/CPA-Manager-Plus) lightweight panel, while full Manager Server mode runs at `cpamp.<DOMAIN>` with persistent request history, usage and cost analytics, model pricing, inspection, and automation. Fronts OAuth providers (Antigravity Gemini, Claude) and OpenAI-compatible upstreams (OpenRouter → GLM) behind a single API key, so agents point at one endpoint instead of juggling per-provider creds. CPAMP connects directly to CPA over `proxy_net`, stores its SQLite database and encryption key under `cpa/manager/`, and uses `CPA_MANAGER_ADMIN_KEY` for login. The retired cpa-usage-keeper data remains under `cpa/keeper/` for recovery but no longer consumes the CPA usage queue. Daily backup of `auths/`, CPAMP data, and the legacy keeper DB to TrueNAS. **OAuth logins are done off-box:** the panel/CLI callback binds a loopback redirect to the proxy's own port, which doesn't resolve against a headless host — so log in locally, then `scp` the resulting `auths/*.json` (each carries a `refresh_token`, so CPA auto-refreshes indefinitely with no further callbacks). Config lives in `config.yaml` (gitignored — holds the mgmt key, client API key, and OpenRouter key); recreate it from `config.example.yaml`. Also mirrored in the [agent-station](https://github.com/masolnada/agent-station) repo for local dev.
- 🌱 **Garden** — Hort, the dashboard for the [automated fertigation system](https://github.com/masolnada/automated-fertigation-system), at `hort.<DOMAIN>`. Two containers built from that repo's `web/` workspace: `hort`, a static nginx artifact (no MQTT, no env), and `hort-server`, an Express-on-Bun service that is the only MQTT client — it connects to the broker over `proxy_net` at `mqtt://mosquitto:1883` and exposes `/api/*` (an SSE stream plus command endpoints). The browser no longer speaks MQTT-over-WebSockets; it hits the single origin `hort.<DOMAIN>`, where Caddy path-splits `/api/*` → `hort-server:4000` (SSE unbuffered) and everything else → `hort:80`. Stateless, so no backup sidecar.
...
graph LR
    ...
    Caddy -->|HTTP /api proxy_net| HortServer[hort-server]
    HortServer -->|MQTT proxy_net| Mosquitto[Mosquitto broker]
    Caddy -->|WSS proxy_net| Mosquitto
...
| Variable | Description |
|---|---|
| `MQTT_USERNAME` | Mosquitto broker username (used by `hort-server`) |
| `MQTT_PASSWORD` | Mosquitto broker password (used by `hort-server`) |

> **Note**: the MQTT credentials live only on `hort-server`, which connects to `mosquitto:1883` internally over `proxy_net`. The browser never sees them — it only talks to `hort-server` over `/api/*` through Caddy.

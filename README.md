...
| `HERMES_DASH_HASH` | bcrypt hash for Caddy `basic_auth` on `hermes.<DOMAIN>` (generate with `caddy hash-password`) — auth moved here from the hermes dashboard's own `basic_auth` so the Workspace can fetch dashboard session tokens over `proxy_net` |
